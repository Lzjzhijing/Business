...
class PracticalSmokeSolver:
    ...
    def optimize_drone_mission(self, drone_idx, arrival_times):
        ...
        target_missile_idx = 0
        for missile_idx in range(self.num_missiles):
            dist = self.distance(drone_pos, self.missiles[missile_idx])
            if dist < min_distance:
                min_distance = dist
                target_missile_idx = missile_idx
        
        # 计算指向目标导弹的航向
        heading = self.calculate_heading_to_target(drone_pos, self.missiles[target_missile_idx])
        
        # 选择合适的速度
        speed = self.drone_speed_max
        
        # 计算烟幕弹投放时间
        smoke_schedule = self.calculate_smoke_schedule(drone_idx, heading, speed, arrival_times)
        
        print(f"无人机FY{drone_idx+1}: 航向={heading:.0f}°, 速度={speed}m/s, 目标导弹=M{target_missile_idx+1}")
...
def main():
    """主函数"""
    print("实用版无人机烟幕干扰弹投放策略优化求解器")
    print("="*50)
    
    # 创建求解器
    solver = PracticalSmokeSolver()
