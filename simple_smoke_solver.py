...
class SimpleSmokeSolver:
    ...
    def find_intersection(self, drone_pos, drone_heading, drone_speed, 
        ...
        # 导弹方向向量
        missile_direction = [missile_target[i] - missile_pos[i] for i in range(3)]
        ...
        # 建立线性方程组
        # drone_pos[0] + drone_direction[0] * drone_speed * t = missile_pos[0] + missile_direction[0] * missile_speed * t
        # drone_pos[1] + drone_direction[1] * drone_speed * t = missile_pos[1] + missile_direction[1] * missile_speed * t
        
        # 整理得: (drone_direction[0] * drone_speed - missile_direction[0] * missile_speed) * t = missile_pos[0] - drone_pos[0]
        #         (drone_direction[1] * drone_speed - missile_direction[1] * missile_speed) * t = missile_pos[1] - drone_pos[1]
        ...
        if t > 0:
            # 计算交点位置
            intersect_pos = [drone_pos[i] + drone_direction[i] * drone_speed * t for i in range(3)]
            return t, intersect_pos
        else:
            return -1, [0, 0, 0]
    ...
    def optimize_drone_mission(self, drone_idx):
        ...
        # 尝试不同的航向和速度组合
        for heading in range(0, 360, 30):  # 每30度一个候选
            for speed in range(self.drone_speed_min, self.drone_speed_max + 1, 20):
                # 计算该配置下的最优烟幕弹投放时间
    ...
    def optimize_smoke_timing(self, drone_idx, heading, speed, arrival_times):
        ...
        for missile_idx in range(self.num_missiles):
            t_intersect, pos_intersect = self.find_intersection(
                drone_pos, heading, speed,
                self.missiles[missile_idx], self.fake_target, self.missile_speed)
            
            if t_intersect > 0 and t_intersect <= 100:  # 最大仿真时间100s
        ...
        for i, intersect_time in enumerate(intersection_times[:self.max_smoke_per_drone]):
            smoke_time = intersect_time - 2  # 提前2秒投放
        ...
        # 简化的覆盖率计算
        coverage = min(len([t for t in smoke_schedule if t > 0]) / self.num_missiles, 1.0)
    ...
    def generate_results(self, solution):
        ...
        for i in range(self.num_drones):
            for j in range(self.max_smoke_per_drone):
                ...
                if smoke_time > 0:
                    ...
                    drop_pos = [self.drones[i][k] + direction[k] * speed * smoke_time for k in range(3)]
        ...
        # 保存为CSV文件
        with open('drone_info.csv', 'w', newline='', encoding='utf-8') as f:
            if drone_data:
                writer = csv.DictWriter(f, fieldnames=drone_data[0].keys())
                writer.writeheader()
                writer.writerows(drone_data)
        
        with open('smoke_info.csv', 'w', newline='', encoding='utf-8') as f:
            if smoke_data:
    ...
    def print_summary(self, solution, drone_data, smoke_data):
        ...
        for i, smoke in enumerate(smoke_data):
            print(f"  {smoke['无人机编号']}: 投放点=({smoke['投放点X(m)']:.0f}, {smoke['投放点Y(m)']:.0f}, {smoke['投放点Z(m)']:.0f}), "
                  f"起爆点=({smoke['起爆点X(m)']:.0f}, {smoke['起爆点Y(m)']:.0f}, {smoke['起爆点Z(m)']:.0f}), "
        ...
        print(f"  平均投放时间: {avg_drop_time:.1f}s")
