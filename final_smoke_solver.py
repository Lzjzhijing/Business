...
class FinalSmokeSolver:
    ...
    def calculate_optimal_heading(self, drone_idx, target_missile_idx):
        """计算无人机指向特定导弹的最优航向"""
        drone_pos = self.drones[drone_idx]
        missile_pos = self.missiles[target_missile_idx]
        
        # 计算从无人机到导弹的方向
        dx = missile_pos[0] - drone_pos[0]
        dy = missile_pos[1] - drone_pos[1]
    ...
    def calculate_intercept_time(self, drone_idx, heading, speed, missile_idx):
        """计算无人机拦截导弹的时间"""
        drone_pos = self.drones[drone_idx]
    ...
    def optimize_drone_mission(self, drone_idx):
        ...
        strategies = [
            # 策略1: 拦截M1
            (self.calculate_optimal_heading(drone_idx, 0), self.drone_speed_max),
            # 策略2: 拦截M2
            (self.calculate_optimal_heading(drone_idx, 1), self.drone_speed_max),
            # 策略3: 拦截M3
            (self.calculate_optimal_heading(drone_idx, 2), self.drone_speed_max),
            # 策略4: 指向假目标
            (self.calculate_optimal_heading(drone_idx, 0) if drone_idx == 0 else 0, self.drone_speed_max),
            # 策略5: 指向真目标
            (self.calculate_optimal_heading(drone_idx, 0) if drone_idx == 0 else 0, self.drone_speed_max),
        ]
        
        for heading, speed in strategies:
            # 计算该配置下的烟幕弹投放时间
            smoke_schedule = self.calculate_smoke_schedule(
                drone_idx, heading, speed, arrival_times)
            
            # 计算覆盖率
            coverage = self.calculate_mission_coverage(
                drone_idx, heading, speed, smoke_schedule, arrival_times)
        ...
        print(f"无人机FY{drone_idx+1}: 航向={best_heading:.0f}°, 速度={best_speed}m/s, 覆盖率={best_coverage*100:.2f}%")
    ...
    def calculate_smoke_schedule(self, drone_idx, heading, speed, arrival_times):
        ...
        for missile_idx in range(self.num_missiles):
            intercept_time = self.calculate_intercept_time(drone_idx, heading, speed, missile_idx)
        ...
        # 按时间排序
        intercept_times.sort()
        
        # 分配烟幕弹
        smoke_count = 0
        last_smoke_time = 0
        
        for i, intercept_time in enumerate(intercept_times[:self.max_smoke_per_drone]):
    ...
    def calculate_mission_coverage(self, drone_idx, heading, speed, smoke_schedule, arrival_times):
        """计算任务覆盖率"""
        coverage = 0
        ...
        for smoke_time in smoke_schedule:
            if smoke_time > 0:
                ...
                direction = [math.cos(math.radians(heading)), 
                           math.sin(math.radians(heading)), 0]
                drop_pos = [drone_pos[i] + direction[i] * speed * smoke_time for i in range(3)]
                
                # 检查覆盖的导弹数量
                missile_coverage = 0
                for missile_idx in range(self.num_missiles):
                    # 计算导弹在烟幕弹投放时的位置
                    missile_pos = self.calculate_missile_position_at_time(missile_idx, smoke_time)
                    
                    # 计算烟幕云团位置
                    cloud_pos = self.calculate_smoke_cloud_position(drop_pos, 0)  # 瞬时起爆
                    
                    # 检查是否在有效范围内
                    distance = self.distance(missile_pos, cloud_pos)
                    if distance <= self.smoke_effective_radius:
                        missile_coverage += 1
                
                coverage += missile_coverage
        
        return min(coverage / self.num_missiles, 1.0)
    
    def calculate_missile_position_at_time(self, missile_idx, time):
        """计算导弹在指定时间的位置"""
...
def main():
    """主函数"""
    print("最终版无人机烟幕干扰弹投放策略优化求解器")
    print("="*50)
    
    # 创建求解器
    solver = FinalSmokeSolver()
    
    # 计算导弹到达时间
    arrival_times = solver.calculate_missile_arrival_times()
    print(f"导弹到达时间: M1={arrival_times[0]:.1f}s, M2={arrival_times[1]:.1f}s, M3={arrival_times[2]:.1f}s")
