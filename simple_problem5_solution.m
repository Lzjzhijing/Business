function simple_problem5_solution()
    % 问题5简化解决方案：直接计算合理的烟幕干扰弹投放策略
    ...
    % 使用简化的启发式方法生成解
    [drone_speeds, drop_times, detonation_times] = generate_simple_solution(drones, missiles, fake_target, missile_speed);
    
    % 计算有效遮蔽时间
    effective_time = calculate_simple_coverage(drone_speeds, drop_times, detonation_times, ...
    ...
    % 生成最终结果
    generate_simple_results(drone_speeds, drop_times, detonation_times, drones, ...
                           missiles, fake_target, smoke_effective_radius, smoke_effective_time, ...
                           g, smoke_sink_speed, missile_speed, effective_time);
end

function [drone_speeds, drop_times, detonation_times] = generate_simple_solution(drones, missiles, target, missile_speed)
    % 生成简化的合理解
    ...
    for i = 1:n_drones
        ...
        drone_speeds(i) = 80 + 40 * speed_factor; % 80-120 m/s
    end
    ...
    for drone_idx = 1:n_drones
        for smoke_idx = 1:3
            ...
            % 投放时间：在最早导弹到达前15秒开始投放
            base_time = min(missile_times) - 15;
            drop_times(smoke_global_idx) = base_time + (drone_idx-1)*3 + (smoke_idx-1)*1.5;
...
function effective_time = calculate_simple_coverage(drone_speeds, drop_times, detonation_times, ...
    ...
    for t_idx = 1:length(time_steps)
        ...
        % 计算每枚导弹在时刻t的位置
        for missile_idx = 1:3
            missile_pos = calculate_simple_missile_position(missiles(missile_idx, :), target, missile_speed, t);
            ...
            for smoke_idx = 1:n_smoke_total
                ...
                if drop_times(smoke_idx) <= t && detonation_times(smoke_idx) <= t
                    ...
                    if t <= detonation_times(smoke_idx) + smoke_effective_time
                        % 计算烟幕云团位置
                        smoke_pos = calculate_simple_smoke_position(drones(drone_idx, :), ...
    ...
    effective_time = 0;
    for missile_idx = 1:3
        covered_periods = find_simple_consecutive_periods(coverage_matrix(:, missile_idx));
...
function missile_pos = calculate_simple_missile_position(initial_pos, target_pos, speed, t)
    % 简化的导弹位置计算
...
function smoke_pos = calculate_simple_smoke_position(drone_pos, drone_speed, drop_time, ...
...
function periods = find_simple_consecutive_periods(coverage_vector)
    % 找到连续的遮蔽时间段
    periods = [];
    start_idx = 0;
...
function generate_simple_results(drone_speeds, drop_times, detonation_times, drones, ...
    ...
    for drone_idx = 1:5
        for smoke_idx = 1:3
            ...
            % 计算投放位置
            drop_pos = calculate_simple_drop_position(drones(drone_idx, :), drone_speeds(drone_idx), ...
            ...
            % 计算起爆位置
            detonation_pos = calculate_simple_detonation_position(drones(drone_idx, :), drone_speeds(drone_idx), ...
            ...
            % 计算有效干扰时长
            interference_time = calculate_simple_interference_time(smoke_global_idx, drone_idx, drone_speeds(drone_idx), ...
                                                                  drop_times(smoke_global_idx), detonation_times(smoke_global_idx), ...
                                                                  drones, missiles, target, smoke_effective_radius, ...
                                                                  smoke_effective_time, g, smoke_sink_speed, missile_speed);
            ...
            % 确定干扰的导弹编号
            interfered_missiles = find_simple_interfered_missiles(smoke_global_idx, drone_idx, drone_speeds(drone_idx), ...
                                                               drop_times(smoke_global_idx), detonation_times(smoke_global_idx), ...
                                                               drones, missiles, target, smoke_effective_radius, ...
                                                               smoke_effective_time, g, smoke_sink_speed, missile_speed);
...
function drop_pos = calculate_simple_drop_position(drone_pos, drone_speed, drop_time, target)
    % 计算投放位置
    drone_direction = target - drone_pos;
    drone_direction = drone_direction / norm(drone_direction);
    drop_pos = drone_pos + drone_direction * drone_speed * drop_time;
...
function detonation_pos = calculate_simple_detonation_position(drone_pos, drone_speed, drop_time, detonation_time, g, target)
    % 计算起爆位置
    drone_direction = target - drone_pos;
    drone_direction = drone_direction / norm(drone_direction);
    drop_pos = drone_pos + drone_direction * drone_speed * drop_time;
...
function interference_time = calculate_simple_interference_time(smoke_idx, drone_idx, drone_speed, drop_time, detonation_time, ...
                                                              drones, missiles, target, smoke_effective_radius, ...
                                                              smoke_effective_time, g, smoke_sink_speed, missile_speed)
    % 计算单个烟幕干扰弹的有效干扰时长
    dt = 0.5;
    ...
    for t_idx = 1:length(time_steps)
        ...
        if drop_time <= t && detonation_time <= t && t <= detonation_time + smoke_effective_time
            % 计算烟幕位置
            smoke_pos = calculate_simple_smoke_position(drones(drone_idx, :), drone_speed, drop_time, ...
...
function interfered_missiles = find_simple_interfered_missiles(smoke_idx, drone_idx, drone_speed, drop_time, detonation_time, ...
                                                             drones, missiles, target, smoke_effective_radius, ...
                                                             smoke_effective_time, g, smoke_sink_speed, missile_speed)
    % 确定被干扰的导弹编号
    dt = 0.5;
