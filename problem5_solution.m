function problem5_solution()
    % 问题5：5架无人机对3枚来袭导弹的烟幕干扰弹投放策略优化
    % 基于PSO算法的烟幕干扰弹投放策略优化
    ...
    smoke_effective_time = 20; % 烟幕有效遮蔽时间 s（起爆后20秒内有效）
    ...
    % 目标函数：最大化有效遮蔽时间
    objective = @(x) -calculate_effective_coverage_time(x, drones, missiles, ...
                                                       fake_target, smoke_effective_radius, ...
                                                       smoke_effective_time, g, smoke_sink_speed, ...
                                                       missile_speed, n_drones);
    
    % 非线性约束
    nonlcon = @(x) problem5_constraints(x, n_drones, n_smoke_total);
    ...
    if exitflag > 0
        ...
        % 生成最终结果
        generate_problem5_results(drone_speeds, drop_times, detonation_times, drones, ...
                                 missiles, fake_target, smoke_effective_radius, smoke_effective_time, ...
                                 g, smoke_sink_speed, missile_speed, n_drones, -fval);
        
        % 可视化结果
        visualize_problem5_results(drone_speeds, drop_times, detonation_times, drones, missiles, ...
...
function [c, ceq] = problem5_constraints(x, n_drones, n_smoke_total)
    % 问题5的约束函数
    ...
    % 约束1：起爆时间必须大于投放时间（至少0.5秒间隔）
    for i = 1:n_smoke_total
        c = [c; drop_times(i) - detonation_times(i) + 0.5];
    end
    
    % 约束2：同一无人机投放的烟幕干扰弹间隔至少2秒
    for drone_idx = 1:n_drones
        for smoke_idx = 1:2
            ...
            if smoke2_idx <= n_smoke_total
                c = [c; 2 - abs(drop_times(smoke1_idx) - drop_times(smoke2_idx))];
            end
        end
    end
...
function effective_time = calculate_effective_coverage_time(x, drones, missiles, ...
    ...
    for t_idx = 1:length(time_steps)
        ...
        % 计算每枚导弹在时刻t的位置
        for missile_idx = 1:3
            missile_pos = calculate_missile_position(missiles(missile_idx, :), target, missile_speed, t);
            ...
            for smoke_idx = 1:n_smoke_total
                ...
                if drop_times(smoke_idx) <= t && detonation_times(smoke_idx) <= t
                    ...
                    if t <= detonation_times(smoke_idx) + smoke_effective_time
                        % 计算烟幕云团位置
                        smoke_pos = calculate_smoke_position(drones(drone_idx, :), ...
...
function missile_pos = calculate_missile_position(initial_pos, target_pos, speed, t)
    % 计算导弹在时刻t的位置
...
function smoke_pos = calculate_smoke_position(drone_pos, drone_speed, drop_time, ...
...
function generate_problem5_results(drone_speeds, drop_times, detonation_times, drones, ...
    ...
    for drone_idx = 1:n_drones
        for smoke_idx = 1:3
            ...
            % 计算投放位置
            drop_pos = calculate_drop_position(drones(drone_idx, :), drone_speeds(drone_idx), ...
            ...
            % 计算起爆位置
            detonation_pos = calculate_detonation_position(drones(drone_idx, :), drone_speeds(drone_idx), ...
            ...
            % 计算有效干扰时长
            interference_time = calculate_interference_time(smoke_global_idx, drone_idx, drone_speeds(drone_idx), ...
                                                          drop_times(smoke_global_idx), detonation_times(smoke_global_idx), ...
            ...
            % 确定干扰的导弹编号
            interfered_missiles = find_interfered_missiles(smoke_global_idx, drone_idx, drone_speeds(drone_idx), ...
                                                         drop_times(smoke_global_idx), detonation_times(smoke_global_idx), ...
                                                         drones, missiles, target, smoke_effective_radius, ...
                                                         smoke_effective_time, g, smoke_sink_speed, missile_speed);
...
function drop_pos = calculate_drop_position(drone_pos, drone_speed, drop_time, target)
    % 计算投放位置
    drone_direction = target - drone_pos;
    drone_direction = drone_direction / norm(drone_direction);
    drop_pos = drone_pos + drone_direction * drone_speed * drop_time;
...
function detonation_pos = calculate_detonation_position(drone_pos, drone_speed, drop_time, detonation_time, g, target)
    % 计算起爆位置
    drone_direction = target - drone_pos;
    drone_direction = drone_direction / norm(drone_direction);
    drop_pos = drone_pos + drone_direction * drone_speed * drop_time;
...
function interference_time = calculate_interference_time(smoke_idx, drone_idx, drone_speed, drop_time, detonation_time, ...
                                                        drones, missiles, target, smoke_effective_radius, ...
                                                        smoke_effective_time, g, smoke_sink_speed, missile_speed)
    % 计算单个烟幕干扰弹的有效干扰时长
    dt = 0.1;
    ...
    for t_idx = 1:length(time_steps)
        ...
        if drop_time <= t && detonation_time <= t && t <= detonation_time + smoke_effective_time
            % 计算烟幕位置
            smoke_pos = calculate_smoke_position(drones(drone_idx, :), drone_speed, drop_time, ...
...
function interfered_missiles = find_interfered_missiles(smoke_idx, drone_idx, drone_speed, drop_time, detonation_time, ...
                                                       drones, missiles, target, smoke_effective_radius, ...
                                                       smoke_effective_time, g, smoke_sink_speed, missile_speed)
    % 确定被干扰的导弹编号
    dt = 0.1;
...
function visualize_problem5_results(drone_speeds, drop_times, detonation_times, drones, missiles, ...
                                   real_target, fake_target, smoke_effective_radius, g, smoke_sink_speed, missile_speed)
    % 可视化问题5的结果
    ...
    for t_idx = 1:length(time_steps)
        t = time_steps(t_idx);
        for missile_idx = 1:3
            missile_pos = calculate_missile_position(missiles(missile_idx, :), fake_target, missile_speed, t);
            is_covered = false;
            for smoke_idx = 1:length(drop_times)
                drone_idx = ceil(smoke_idx / 3);
                if drop_times(smoke_idx) <= t && detonation_times(smoke_idx) <= t
                    if t <= detonation_times(smoke_idx) + 20 % 20秒有效期
                        smoke_pos = calculate_smoke_position(drones(drone_idx, :), drone_speeds(drone_idx), ...
